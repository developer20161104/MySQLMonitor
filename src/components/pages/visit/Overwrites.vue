<template>
  <div>
    ...
    <el-row>
      <div id="Handler_write" style="width: 100%; height: 500px;"></div>
    </el-row>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getMonitorData () {
        const Handler_write = document.getElementById("Handler_write");
        this.chart = echarts.init(Handler_write);
        this.getData()
        setTimeout(() => {
          this.exchange()
          this.setOptions()
      ...
      exchange () {
        if(this.message.expectedData === null){
          ...
          for(let i=0;i<this.message.expectedData.length;i++){
            ...
            this.message.Ydata.push(this.message.expectedData[i].Handler_write);
          }
        }
      }
    },
