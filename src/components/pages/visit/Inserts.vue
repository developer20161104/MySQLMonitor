<template>
  <div>
    ...
    <el-row>
      <div id="Com_insert" style="width: 100%; height: 500px;"></div>
    </el-row>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getMonitorData () {
        const Com_insert = document.getElementById("Com_insert");
        this.chart = echarts.init(Com_insert);
        this.getData()
        setTimeout(() => {
          this.exchange()
          this.setOptions()
      ...
      exchange () {
        if(this.message.expectedData === null){
          ...
          for(let i=0;i<this.message.expectedData.length;i++){
            ...
            this.message.Ydata.push(this.message.expectedData[i].Com_insert);
          }
        }
      }
    },
