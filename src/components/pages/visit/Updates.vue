<template>
  <div>
    ...
    <el-row>
      <div id="Com_update" style="width: 100%; height: 500px;"></div>
    </el-row>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      getMonitorData () {
        ...
        this.chart = echarts.init(Com_update);
        this.getData()
        setTimeout(() => {
          this.exchange()
          this.setOptions()
      ...
      exchange () {
        if(this.message.expectedData === null){
          ...
          for(let i=0;i<this.message.expectedData.length;i++){
            ...
            this.message.Ydata.push(this.message.expectedData[i].Com_update);
          }
        }
      }
    },
