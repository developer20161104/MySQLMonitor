<template>
  <div>
    ...
    <el-row>
      <div id="Slow_queries" style="width: 100%; height: 500px;"></div>
    </el-row>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getMonitorData () {
        const Slow_queries = document.getElementById("Slow_queries");
        this.chart = echarts.init(Slow_queries);
        this.getData()
        setTimeout(() => {
          this.exchange()
          this.setOptions()
      ...
      exchange () {
        if(this.message.expectedData === null){
          ...
          for(let i=0;i<this.message.expectedData.length;i++){
            ...
            this.message.Ydata.push(this.message.expectedData[i].Slow_queries);
          }
        }
      }
    },
