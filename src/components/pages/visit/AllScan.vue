<template>
  <div>
    ...
    <el-row>
      <div id="Full_scan" style="width: 100%; height: 500px;"></div>
    </el-row>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getMonitorData () {
        const Full_scan = document.getElementById("Full_scan");
        this.chart = echarts.init(Full_scan);
        this.getData()
        setTimeout(() => {
          this.exchange()
          this.setOptions()
      ...
      exchange () {
        if(this.message.expectedData === null){
          ...
          for(let i=0;i<this.message.expectedData.length;i++){
            ...
            this.message.Ydata.push(this.message.expectedData[i].Full_scan);
          }
          console.log(this.message.Ydata)
        }
      }
    },
    mounted() {
