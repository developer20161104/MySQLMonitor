<template>
  <div>
    ...
    <el-row>
      <div id="Max_connections" style="width: 100%; height: 500px;"></div>
    </el-row>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      getMonitorData () {
        ...
        this.chart = echarts.init(Max_connections);
        this.getData()
        setTimeout(() => {
          this.exchange()
          this.setOptions()
      ...
      exchange () {
        if(this.message.expectedData === null){
          ...
          for(let i=0;i<this.message.expectedData.length;i++){
            ...
            this.message.Ydata.push(this.message.expectedData[i].Max_connections);
          }
        }
      }
    },
