<template>
  <div>
    ...
    <el-row>
      <div id="Created_tmp_tables" style="width: 100%; height: 500px;"></div>
    </el-row>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getMonitorData () {
        const Created_tmp_tables = document.getElementById("Created_tmp_tables");
        this.chart = echarts.init(Created_tmp_tables);
        this.getData()
        setTimeout(() => {
          this.exchange()
          this.setOptions()
      ...
      exchange () {
        if(this.message.expectedData === null){
          ...
          for(let i=0;i<this.message.expectedData.length;i++){
            ...
            this.message.Ydata.push(this.message.expectedData[i].Created_tmp_tables);
          }
        }
      }
    },
