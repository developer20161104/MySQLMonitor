<template>
  <div>
    ...
    <el-row>
      <div id="Innodb_data_reads" style="width: 100%; height: 500px;"></div>
    </el-row>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getMonitorData () {
        const Innodb_data_reads = document.getElementById("Innodb_data_reads");
        this.chart = echarts.init(Innodb_data_reads);
        this.getData()
        setTimeout(() => {
          this.exchange()
          this.setOptions()
      ...
      exchange () {
        if(this.message.expectedData === null){
          ...
          for(let i=0;i<this.message.expectedData.length;i++){
            ...
            this.message.Ydata.push(this.message.expectedData[i].Innodb_data_reads);
          }
        }
      }
    },
