<template>
  <div>
    ...
    <el-row>
      <div id="Open_tables" style="width: 100%; height: 500px;"></div>
    </el-row>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getMonitorData () {
        const Open_tables = document.getElementById("Open_tables");
        this.chart = echarts.init(Open_tables);
        this.getData()
        setTimeout(() => {
          this.exchange()
          this.setOptions()
      ...
      exchange () {
        if(this.message.expectedData === null){
          ...
          for(let i=0;i<this.message.expectedData.length;i++){
            ...
            this.message.Ydata.push(this.message.expectedData[i].Open_tables);
          }
        }
      }
    },
