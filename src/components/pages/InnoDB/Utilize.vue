<template>
  <div>
    ...
    <el-row>
      <div id="Utilize" style="width: 100%; height: 500px;"></div>
    </el-row>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getMonitorData () {
        const Utilize = document.getElementById("Utilize");
        this.chart = echarts.init(Utilize);
        this.getData()
        setTimeout(() => {
          this.exchange()
          this.setOptions()
      ...
      exchange () {
        if(this.message.expectedData === null){
          ...
          for(let i=0;i<this.message.expectedData.length;i++){
            ...
            this.message.Ydata.push(this.message.expectedData[i].Utilize);
          }
        }
      }
    },
